...
struct Config {
  ...
  std::size_t capacity = 1ULL << 30;
  std::size_t page_size = 0;

  static Config from_env() {
    Config cfg;
    if (auto *v = std::getenv("MAAS_GRPC_ADDR"))
      cfg.grpc_addr = v;
    if (auto *v = std::getenv("MAAS_PG_CONNINFO"))
      cfg.pg_conninfo = v;
    if (auto *v = std::getenv("MAAS_ARENA_SIZE"))
      cfg.capacity = std::stoull(v);
...
struct ShmBlock {
  ...
  std::string tenant_id;     // UUID do tenant dono da alocação
};
...
public:
  ...
  [[nodiscard]] std::optional<AllocResult> allocate(std::size_t size_bytes) {
    ...
    while (true) {
      if (current + aligned > capacity_) {
        std::cerr << "[SHM] Capacidade esgotada: used=" << current
                  << " requested=" << aligned << " capacity=" << capacity_
                  << "\n";
        return std::nullopt;
      }
    ...
    if (ftruncate(fd, static_cast<off_t>(aligned)) == -1) {
      std::cerr << "[SHM] ftruncate falhou para '" << shm_name << "' ("
                << aligned << " bytes): " << std::strerror(errno) << "\n";
      close(fd);
    ...
    void *ptr =
        mmap(nullptr, aligned, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (ptr == MAP_FAILED) {
      std::cerr << "[SHM] mmap falhou para '" << shm_name << "' (" << aligned
    ...
    if (mlock(ptr, aligned) != 0) {
      ...
      std::cerr << "[SHM] mlock falhou para '" << shm_name
                << "' (CAP_IPC_LOCK necessário): " << std::strerror(errno)
                << "\n";
    }
  ...
  bool release(const std::string &allocation_id) {
    ...
    std::cout << "[SHM] Liberado: " << name << " (" << freed << " bytes)\n";
    return true;
  }
  ...
  // -------------------------------------------------------------------------
  bool write(const std::string &tenant_id, const std::string &allocation_id,
             std::size_t offset, const char *data, std::size_t len) {
    std::shared_lock lock(mutex_);
    auto it = blocks_.find(allocation_id);
    if (it == blocks_.end())
      return false;
    ...
    if (blk.tenant_id != tenant_id)
      return false;
    if (offset + len > blk.size)
      return false;
  ...
  // -------------------------------------------------------------------------
  bool read(const std::string &tenant_id, const std::string &allocation_id,
            std::size_t offset, std::size_t len, std::string &out) {
    std::shared_lock lock(mutex_);
    auto it = blocks_.find(allocation_id);
    if (it == blocks_.end())
      return false;
...
public:
  ...
  // -------------------------------------------------------------------------
  [[nodiscard]] std::string insert_allocation(const std::string &tenant_id,
    ...
    const char *params[] = {tenant_id.c_str(), node_id.c_str(), shm_key.c_str(),
                            offset_s.c_str(), size_s.c_str()};
  ...
  [[nodiscard]] std::optional<AllocationMeta>
  release_allocation(const std::string &allocation_id,
                     const std::string &tenant_id) {
    std::lock_guard lock(mutex_);
    ensure_connection();

    const char *params[] = {allocation_id.c_str(), tenant_id.c_str()};
    ...
    PGresult *res =
        PQexecParams(conn_,
                     ...
                     "WHERE allocation_id = $1::uuid AND tenant_id = $2::uuid "
                     "AND state = 'active' "
    ...
    if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) > 0) {
      meta = AllocationMeta{PQgetvalue(res, 0, 0),
  ...
  // -------------------------------------------------------------------------
  bool insert_metric(const std::string &tenant_id, const std::string &node_id,
                     const std::string &allocation_id, double rtt_ms,
                     double cache_hit_ratio, double memory_pressure,
...
public:
  MemoryServiceImpl(ShmManager &shm, DatabaseClient &db,
                    const std::string &node_id)
  ...
  // -------------------------------------------------------------------------
  grpc::Status Allocate(grpc::ServerContext * /*ctx*/,
                        const maas::AllocateRequest *req,
                        maas::AllocateResponse *resp) override {
    ...
    std::string alloc_id =
        db_.insert_allocation(req->tenant_id(), node_id_, result->shm_name,
    ...
    if (alloc_id.empty()) {
      ...
      return grpc::Status(grpc::INTERNAL,
                          "Falha ao persistir alocação no banco");
    }

    // 3. Registra no mapa interno para lookup no Deallocate
    ShmBlock blk{result->shm_name,     result->fd, result->ptr,
                 result->aligned_size, alloc_id,   req->tenant_id()};
    ...
    std::cout << "[ALLOC] tenant=" << req->tenant_id()
              << " size=" << req->size_bytes() << " shm=" << result->shm_name
              << " id=" << alloc_id << "\n";
  ...
  // -------------------------------------------------------------------------
  grpc::Status Deallocate(grpc::ServerContext * /*ctx*/,
                          const maas::DeallocateRequest *req,
                          maas::DeallocateResponse *resp) override {
    if (req->tenant_id().empty()) {
      return grpc::Status(grpc::INVALID_ARGUMENT, "tenant_id é obrigatório");
    }
    if (req->allocation_id().empty()) {
      return grpc::Status(grpc::INVALID_ARGUMENT,
                          "allocation_id é obrigatório");
    }

    // 1. Marca como 'released' no banco (atômico: só altera se state='active')
    auto meta = db_.release_allocation(req->allocation_id(), req->tenant_id());
    if (!meta) {
    ...
    std::cout << "[DEALLOC] id=" << req->allocation_id()
              << " shm=" << meta->shm_key << " size=" << meta->size_bytes
              << "\n";
  ...
  grpc::Status ReportMetrics(grpc::ServerContext * /*ctx*/,
                             ...
                             maas::Acknowledge *resp) override {
    maas::MetricsReport report;
    int64_t count = 0;

    while (reader->Read(&report)) {
      db_.insert_metric(report.tenant_id(), report.node_id(),
                        report.allocation_id(), report.rtt_ms(),
                        report.cache_hit_ratio(), report.memory_pressure(),
  ...
  // -------------------------------------------------------------------------
  grpc::Status WriteMemory(grpc::ServerContext * /*ctx*/,
    ...
    bool ok = shm_.write(req->tenant_id(), req->allocation_id(), req->offset(),
    ...
    if (!ok) {
      ...
      if (!shm_.has_block(req->allocation_id())) {
        std::cerr
            << "[DEBUG] Motivo: ID não encontrado no mapa local do servidor."
            << std::endl;
      }
      return grpc::Status(grpc::OUT_OF_RANGE,
                          "Erro ao escrever na memória (ID inválido ou offset "
                          "fora dos limites)");
    }
  ...
  // -------------------------------------------------------------------------
  grpc::Status ReadMemory(grpc::ServerContext * /*ctx*/,
                          const maas::ReadRequest *req,
                          maas::ReadResponse *resp) override {
    if (req->tenant_id().empty()) {
      return grpc::Status(grpc::INVALID_ARGUMENT, "tenant_id é obrigatório");
    }
    if (req->allocation_id().empty()) {
    ...
    std::string buffer;
    bool ok = shm_.read(req->tenant_id(), req->allocation_id(), req->offset(),
                        req->size_bytes(), buffer);
    if (!ok) {
      std::cerr << "[DEBUG] Erro de Leitura: ID=" << req->allocation_id()
                << ", offset=" << req->offset()
                << ", size=" << req->size_bytes() << std::endl;
      if (!shm_.has_block(req->allocation_id())) {
        std::cerr
            << "[DEBUG] Motivo: ID não encontrado no mapa local do servidor."
            << std::endl;
      ...
      return grpc::Status(
          grpc::OUT_OF_RANGE,
          "Erro ao ler da memória (ID inválido ou offset fora dos limites)");
    }
  ...
  // -------------------------------------------------------------------------
  grpc::Status CheckHealth(grpc::ServerContext * /*ctx*/,
                           const maas::Empty * /*req*/,
                           maas::HealthCheckResponse *resp) override {
...
int main() {
  ...
  std::cout << "[CONFIG] addr=" << cfg.grpc_addr << " capacity=" << cfg.capacity
            << "B"
  ...
  for (int attempt = 1; attempt <= 10; ++attempt) {
    if (db.connect())
      break;
    std::cerr << "[DB] Tentativa " << attempt
              << "/10 falhou, aguardando 2s...\n";
    sleep(2);
  }

  // 4. Identificador deste nó
  //    Stub fixo — em produção, registrar via INSERT INTO ClusterNode RETURNING
  //    node_id
